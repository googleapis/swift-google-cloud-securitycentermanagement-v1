# Google Cloud Client Libraries for Swift - Security Command Center Management API
...
Add `swift-google-cloud-securitycentermanagement-v1` as a package dependency:

```bash
swift package add-dependency https://github.com/googleapis/swift-google-cloud-securitycentermanagement-v1.git --from 0.0.0-preview
```

Then add `GoogleCloudSecurityCenterManagementV1` to your target's dependencies:

```bash
swift package add-target-dependency GoogleCloudSecurityCenterManagementV1 <target-name> --package swift-google-cloud-securitycentermanagement-v1
```
